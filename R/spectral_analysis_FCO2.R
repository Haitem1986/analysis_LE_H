...
#df_FCO2 <- data.frame(date = df$date, FCO2 = df$H)
df_FCO2 <- data.frame(FCO2 = df$co2_flux)

# Convert data to a time series object
df_ts <- ts(df_FCO2, frequency = 1)
...
prominence_threshold <- 0.005  # Example threshold value
